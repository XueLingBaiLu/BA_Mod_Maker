...
# BA Mod Maker — 断箭 Mod 制作工具 v1.8.55
...
配套 Blender 插件（`BA_Mod_Maker_blender_addon.zip`，v2.7.55）负责模型侧：
...
- **枚举绿色箭头 ➜**：单位大类 `Type` / 槽位类别 `CategoryType` / 角色 `Role`（以及武器类型、弹道类型等
  全部枚举字段）右侧直接显示该 ID 对应的种类（`Role=11` → 绿色 `➜ 主战坦克 (Tank)`），点箭头跳词典对照表；
  未知值黄色告警（游戏会走兜底分支，不会崩）
